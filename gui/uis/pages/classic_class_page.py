...
from PySide6.QtGui import QIcon, Qt, QPixmap, QFont, QColor, QImage, QPainter, QPen, QRegion, QGuiApplication
...
class Ui_ClassicClassPage(BaseClassPage):
    def __init__(self, main_window: QMainWindow):
        # 调用基类初始化，指定游戏版本为 classic
        super().__init__(main_window, game_version='classic')
    ...
    def setupUi(self, page_skills):
        ...
        self.full_screen_label = CaptureLabel(self)
    ...
    def save_rotation_config(self, config):
        ...
        # 计算项目根目录的绝对路径
        # __file__ 是 gui/uis/pages/classic_class_page.py，需要向上4级到项目根目录
    ...
    def _init_hdr_from_config(self):
        """仅在 UI 初始化时调用一次，用配置中的 HDR 亮度同步到控件（经典服）。
    ...
    def _init_template_scale_from_config(self):
        """仅在 UI 初始化时调用一次，为缩放控件设置默认值（经典服）。

        注意：缩放值只保存在对应职业/天赋的 classic_config JSON 中，
    ...
    def _load_preview_templates(self):
        ...
        - 使用当前 class + talent 对应的图标目录：
          gui/uis/icons/classic/talent_icons/<class>/<talent>/
        """
        ...
        templates_dir = os.path.join(gui_dir, "uis", "icons", "classic", "talent_icons", class_name, talent_name)
        if not os.path.isdir(templates_dir):
            print(f"[Classic] 未找到天赋图标目录: {templates_dir}")
            return []
        ...
        if not loaded:
            print(f"[Classic] 天赋目录中未找到可用的图标: {templates_dir}")
    ...
    def _match_best_icon(self, frame_bgr):
        ...
        # 将 (name, tmpl_bgr) 列表转换为 {name: tmpl_bgr} 字典，便于共用算法
    ...
    def _apply_hdr_correction(self, frame_bgr):
        """
        针对开启 HDR 时截图偏亮的问题，对截图做一次「色调映射 + 可调压暗」（经典服预览）。

        实现完全复用 `TemplateMatcher.apply_hdr_correction`，使用当前 UI 中的
    ...
    def _save_template_scale_to_config(self):
        ...
        写入位置：
        - 当前职业/天赋对应的 classic_config json 中的 zoom 字段（对应 config）
        """
        current_scale = float(getattr(self, "template_scale", 1.0))
        # 仅写入当前职业/天赋对应的 classic_config/<Class>_<Talent>.json
        if self.selected_class_name and self.selected_talent_name:
            ...
            if os.path.exists(config_filepath):
                try:
                    ...
                    print(f"[Classic] 读取缩放配置文件失败 {config_filepath}: {e}", flush=True)
            ...
            try:
                with open(config_filepath, 'w', encoding='utf-8') as file:
                    ...
                    # print(f"[Classic] 缩放倍率已写入到 {config_filepath} 中的 zoom 字段: {current_scale}")
            except Exception as e:
                print(f"[Classic] 写入缩放到配置文件失败 {config_filepath}: {e}", flush=True)
    ...
    def _save_hdr_to_config(self):
        """将当前 HDR 亮度系数写入配置（经典服）。

        写入位置：
        - rotation/rotation_config.yaml 中的 hdr_darkness 字段（全局默认）
        - 当前职业/天赋对应的 classic_config json 中的 hdr_darkness 字段（对应 config）
        """
        ...
        try:
            ...
            print(f"[Classic] 写入 HDR 亮度配置失败: {e}", flush=True)

        # 2) 写入当前职业/天赋对应的 classic_config/<Class>_<Talent>.json
        if self.selected_class_name and self.selected_talent_name:
            config_filename = f"{self.selected_class_name}_{self.selected_talent_name}.json"
            config_filepath = os.path.join(self.config_folder, config_filename)
            ...
            if os.path.exists(config_filepath):
                try:
                    ...
                    print(f"[Classic] 读取 HDR 配置文件失败 {config_filepath}: {e}", flush=True)
            ...
            try:
                with open(config_filepath, 'w', encoding='utf-8') as file:
                    json.dump(existing_config, file, indent=4, ensure_ascii=False)
                    # print(f"[Classic] HDR 亮度已写入到 {config_filepath} 中的 hdr_darkness 字段: {current_hdr}")
            except Exception as e:
                print(f"[Classic] 写入 HDR 亮度到配置文件失败 {config_filepath}: {e}", flush=True)
        ...
        for i in range(self.talent_ability_layout.count()):
            ...
            if layout:
                ...
                if ability_name and shortcut:
                    shortcut_lower = shortcut.lower()  # 将快捷键转换为小写
                    
                    # 检查 config_data 中是否已有阈值设置
                    if ability_name in self.config_data:
                        config_value = self.config_data[ability_name]
                        # 如果是列表格式 [shortcut, threshold]，保留阈值
                        if isinstance(config_value, list) and len(config_value) >= 2:
                            existing_config[ability_name] = [shortcut_lower, config_value[1]]
                        else:
                            # 否则只保存快捷键（旧格式兼容）
                            existing_config[ability_name] = shortcut_lower
                    else:
                        # 如果 config_data 中没有，检查 existing_config 中是否有阈值
                        if ability_name in existing_config:
                            old_value = existing_config[ability_name]
                            # 如果是列表格式，更新快捷键但保留阈值
                            if isinstance(old_value, list) and len(old_value) >= 2:
                                existing_config[ability_name] = [shortcut_lower, old_value[1]]
                            else:
                                existing_config[ability_name] = shortcut_lower
                        else:
                            # 新技能，只保存快捷键
                    ...
                    # 如果只有技能名称但没有快捷键，检查 config_data 中是否有阈值设置
                    # 这种情况下，如果用户设置了阈值但还没有设置快捷键，我们也应该保存阈值
                    if ability_name in self.config_data:
                        config_value = self.config_data[ability_name]
                        # 如果是列表格式且第一个元素为空字符串（只有阈值），保留到 existing_config
                        if isinstance(config_value, list) and len(config_value) >= 2 and config_value[0] == "":
                            # 只保存阈值，快捷键为空时不保存到文件（因为 matcher 需要快捷键）
                            # 但我们可以保留在 existing_config 中，等用户设置快捷键后再保存
                            pass  # 暂时不保存，等用户设置快捷键
    ...
    def load_config_from_file(self, filepath):
        ...
        try:
            with open(filepath, 'r', encoding='utf-8') as file:
                ...
                # 如果配置中带有 zoom 字段，则用它初始化缩放控件（经典服）
                ...
                if zoom_val is not None:
                    try:
                        ...
                        # 检查scale是否过大
                        self._check_scale_warning()
                        # print(f"[Classic] 已从 {filepath} 中的 zoom 初始化缩放倍率: {scale}")
                    except Exception as e:
                        print(f"[Classic] 解析 zoom 字段失败: {e}", flush=True)

                # 如果配置中带有 hdr_darkness 字段，则用它初始化 HDR 控件（经典服）
                ...
                if hdr_val is not None:
                    try:
                        ...
                        self.hdr_spin.blockSignals(False)
                        # print(f"[Classic] 已从 {filepath} 中的 hdr_darkness 初始化 HDR 亮度: {v}")
                    except Exception as e:
                        print(f"[Classic] 解析 hdr_darkness 字段失败: {e}", flush=True)
    ...
    def toggle_start_pause(self):
        ...
        if not self.is_running:
            ...
            if self.rotation_thread and self.rotation_thread.isRunning():
                ...
                # Update button appearance immediately (thread will finish asynchronously)
                self.start_button.setText("Start")
    ...
    def load_abilities(self, layout, directory, columns=6):
        ...
        # Clear icon widgets dictionary and remove any highlights BEFORE clearing layout
        # This ensures widgets are still valid when removing highlights
        ...
        for i, icon_path in enumerate(abilities):
            ...
            if ability_name in self.config_data:
                ...
                if isinstance(config_value, list) and len(config_value) >= 1:
                    shortcut = config_value[0]
                elif isinstance(config_value, str):
                    shortcut = config_value
                else:
                    shortcut = None
    ...
    def create_add_icon_widget(self, icon='icon_heart.svg'):
        """Creates a widget with an empty icon placeholder and an 'ADD' button."""
        # 调用基类方法，使用 "ADD" 作为按钮文本
    ...
    def remove_highlight(self, icon_name):
        ...
        if icon_name in self.icon_widgets:
            ...
            try:
                ...
                if widget is not None and hasattr(widget, 'setGraphicsEffect'):
                    widget.setGraphicsEffect(None)
            except RuntimeError:
                # Widget has been deleted, remove from dictionary
                if icon_name in self.icon_widgets:
                    del self.icon_widgets[icon_name]
                return
